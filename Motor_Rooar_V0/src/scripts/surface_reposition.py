...
class SurfaceReposition:
    ...
    def surface_reposition(surface_base, rect, surface_rect):
        ...
        if rect.x < 0:
            surface_rect.width = max(0, min(rect.x + rect.width, surface_base.get_width()))
        else:
            surface_rect.width = max(0, min(rect.width, surface_base.get_width() - rect.x))

        # y
        surface_rect.y = min(max(0, surface_rect.y), surface_base.get_height())

        if rect.y < 0:
            surface_rect.height = max(0, min(rect.y + rect.height, surface_base.get_height()))
        else:
            surface_rect.height = max(0, min(rect.height, surface_base.get_height() - rect.y))

        return surface_base.subsurface(surface_rect)
